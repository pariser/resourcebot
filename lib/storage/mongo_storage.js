...
module.exports = function(config) {
  ...
  var resources_db = db(config.mongo_uri).get('resources');
    ...
    resources: {
      ...
      findOne: function(query, cb) {
        ...
        resources_db.find(query, function(err, data) {
          console.log('find, err:', err, ', data:', data);
          if (err) {
            return cb(err, data);
          }
          if (data.length === 0) {
      ...
      save: function(resource_data, cb) {
        resources_db.findAndModify({id: resource_data.id}, resource_data, {upsert: true, new: true}, cb);
      },
      all: function(cb) {
